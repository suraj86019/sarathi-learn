/**
 * Create Admin Modal
 * Modal component for creating new admin users
 */

import React, { useState, useEffect } from 'react';
import { X } from 'lucide-react';
import toast from 'react-hot-toast';
import adminService, { CreateAdminData } from '@/services/admin.service';
...
export default function CreateAdminModal({ isOpen, onClose, onSuccess }: CreateAdminModalProps) {
  ...
  const [formData, setFormData] = useState<CreateAdminData>({
    email: '',
    phone: '',
    password: '', // Will be auto-filled from employee_id
    first_name: '',
    last_name: '',
    school_id: '',
    employee_id: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Admin created successfully! Default password is Employee ID.');
      onSuccess();
      onClose();
      // Reset form
      setFormData({
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              placeholder="e.g., Principal, Vice Principal, Admin Officer"
              className="w-full px-4 py-2 border-2 border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            />
          </div>
