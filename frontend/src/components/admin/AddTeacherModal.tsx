...
import adminDashboardService, { CreateTeacherData } from '../../services/adminDashboard.service';
...
export default function AddTeacherModal({ isOpen, onClose, onSuccess, schoolId, schoolName }: AddTeacherModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await adminDashboardService.addTeacher(formData);
      onSuccess();
      onClose();
      setFormData({
        email: '',
      ...
      <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-2xl max-h-[95vh] sm:max-h-[90vh] overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between">
          <div className="flex items-center gap-2 sm:gap-3 text-white">
            ...
            <h2 className="text-base sm:text-xl font-bold">Add New Teacher</h2>
          </div>
          <button
            onClick={onClose}
        ...
        {/* School Info */}
        {schoolName && (
          <div className="bg-blue-50 px-4 sm:px-6 py-2 sm:py-3 border-b border-blue-100">
            <p className="text-blue-700 text-xs sm:text-sm">Adding teacher to: <span className="font-semibold">{schoolName}</span></p>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 overflow-y-auto max-h-[calc(95vh-100px)] sm:max-h-[calc(90vh-140px)]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div className="sm:col-span-2 mt-2 sm:mt-4">
              <h3 className="text-xs sm:text-sm font-semibold text-gray-700 uppercase tracking-wide mb-2 sm:mb-3">Professional Information</h3>
            </div>

            <div>
              <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">Employee ID *</label>
              <input
                type="text"
                name="employee_id"
            ...
            <div>
              <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">Qualification</label>
              <select
                name="qualification"
            ...
            <div>
              <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">Experience (Years)</label>
              <input
                type="number"
          ...
          <div className="flex items-center justify-end gap-3 sm:gap-4 mt-6 sm:mt-8 pt-4 sm:pt-6 border-t border-gray-200">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex items-center gap-2 px-4 sm:px-6 py-2.5 sm:py-3 text-sm sm:text-base bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-semibold rounded-lg hover:from-blue-700 hover:to-indigo-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg"
            >
              {loading ? (
                <>
